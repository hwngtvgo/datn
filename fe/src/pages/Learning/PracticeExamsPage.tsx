...
export default function PracticeExamsPage() {
  ...
  const [practiceExams, setPracticeExams] = useState<EnhancedExam[]>([]);
  const [filteredExams, setFilteredExams] = useState<EnhancedExam[]>([]);
    ...
    const loadActiveExams = async () => {
      try {
        ...
        if (Array.isArray(response)) {
          ...
          setPracticeExams(enhancedExams);
  ...
  useEffect(() => {
    ...
    // Chỉ hiển thị các loại đề thi GRAMMAR_ONLY, VOCABULARY_ONLY và PRACTICE
    result = result.filter(test => 
  ...
  }, [searchQuery, difficultyFilter, typeFilter, practiceExams]);
  ...
  const loadExamQuestions = async (examId: number, index: number) => {
    ...
    const practiceIndex = practiceExams.findIndex(test => test.id === examId);
    if (practiceIndex === -1) return;
    
    // Nếu đã có dữ liệu, chỉ cần toggle hiển thị
    if (practiceExams[practiceIndex].questionGroups) {
      const updatedTests = [...practiceExams];
  ...
  return (
    <div className="container py-10">
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold tracking-tight mb-2">Bài Tập Luyện TOEIC - {levelTitle}</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Nâng cao kỹ năng ngữ pháp và từ vựng với các bài tập theo chủ đề. Học và luyện tập cùng lúc để nâng cao trình độ TOEIC hiệu quả.
        </p>
      </div>
        ...
        <div className="flex gap-2">
          ...
          <Select value={typeFilter} onValueChange={setTypeFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="PRACTICE">Luyện tập tổng hợp</SelectItem>
            </SelectContent>
          </Select>
      ...
      {loading ? (
        <div className="flex justify-center items-center py-20">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-lg">Đang tải danh sách bài tập...</p>
          </div>
        </div>
      ) : filteredExams.length === 0 ? (
        <div className="text-center py-10">
          <p className="text-lg">Không tìm thấy bài tập nào phù hợp với bộ lọc.</p>
            ...
            <Card key={test.id} className="flex flex-col">
              ...
              <CardContent className="flex-grow">
                <div className="grid grid-cols-2 gap-2 text-sm mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-muted-foreground">Loại bài tập:</span>
                    <span className="font-medium">{test.type || 'PRACTICE'}</span>
                  </div>
                  {test.createdAt && (
              ...
              <CardFooter>
                <Button asChild className="w-full">
                  <Link to={`/practice-tests/${test.id}`}>Làm Bài Tập</Link>
                </Button>
              </CardFooter>
            </Card>
          ))}
